...
const AIDevAssistant: React.FC = () => {
  ...
  const initializeBoltDiyIntegration = async () => {
    try {
      // Controlla se boltDiyIntegration è disponibile
      if (typeof window !== 'undefined' && (window as any).boltDiyIntegration) {
        const initialized = await (window as any).boltDiyIntegration.initialize();
        ...
        addSystemMessage('ℹ️ bolt.diy non configurato. Funzionalità AI base disponibili.', 'info');
      }
    } catch (error) {
      console.error('Errore inizializzazione bolt.diy:', error);
      addSystemMessage('ℹ️ bolt.diy non disponibile. Funzionalità AI base attive.', 'info');
    }
  };

  const checkSystemStatus = async () => {
    try {
      // Controlla bolt.diy (opzionale)
      const boltResponse = await fetch('http://localhost:5174/api/health', {
        signal: AbortSignal.timeout(2000) // Timeout di 2 secondi
      }).catch(() => null);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiMessage: Message = {
        id: Date.now().toString(),
        text: aiResponse || generateLocalResponse(userMessage.text),
        sender: 'ai',
        timestamp: new Date()
      };
