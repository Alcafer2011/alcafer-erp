/*
  # Schema completo per Alcafer ERP

  1. Tabelle principali
    - `users` - Utenti del sistema con ruoli
    - `clienti` - Clienti aziendali
    - `preventivi` - Preventivi per i clienti
    - `lavori` - Lavori in corso e completati
    - `materiali_metallici` - Materiali metallici utilizzati
    - `materiali_vari` - Altri materiali e consumabili
    - `leasing_strumentali` - Attrezzature in leasing
    - `costi_utenze` - Costi di elettricità, acqua, gas
    - `prezzi_materiali` - Prezzi aggiornati dei materiali
    - `manovalanza` - Costi del personale
...
-- Inserimento dati iniziali per costi utenze (senza colonna ditta che non esiste)
