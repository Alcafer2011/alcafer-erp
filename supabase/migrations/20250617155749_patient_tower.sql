/*
  ...
  1. New Tables
    - `fornitori`
      ...
      - `tipo_fornitore` (text, default 'materiali')
  ...
  3. Indexes
    - Performance indexes on nome, tipo_fornitore, attivo, valutazione, partita_iva

  4. Triggers
    - Auto-update updated_at column
    - Audit logging if function exists

  5. Sample Data
    - Insert example suppliers for different categories

  6. Views
    - fornitori_with_stats: Suppliers with order statistics
*/
...
-- Aggiunta constraint per partita_iva unique se non esiste
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fornitori_partita_iva_key' 
    AND table_name = 'fornitori'
  ) THEN
    ALTER TABLE fornitori ADD CONSTRAINT fornitori_partita_iva_key UNIQUE (partita_iva);
  END IF;
END $$;

-- Aggiunta constraint per tipo_fornitore se non esiste
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- Aggiunta constraint per valutazione se non esiste
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- Policy per utenti autenticati (lettura e scrittura) - drop e ricrea se esiste
...
-- Funzione per aggiornamento automatico di updated_at (se non esiste)
...
BEGIN
  ...
  DROP TRIGGER IF EXISTS audit_fornitori ON fornitori;
  
  -- Create new trigger if audit function exists
...
-- Vista per fornitori con statistiche (drop e ricrea)
DROP VIEW IF EXISTS fornitori_with_stats;
CREATE VIEW fornitori_with_stats AS
SELECT 
  f.*,
