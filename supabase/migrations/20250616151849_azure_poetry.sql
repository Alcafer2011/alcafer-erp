...
-- Assicuriamoci che la colonna telefono esista nella tabella clienti
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'clienti' AND column_name = 'telefono'
  ) THEN
    ALTER TABLE clienti ADD COLUMN telefono text;
  END IF;
END $$;

-- Assicuriamoci che la colonna indirizzo esista nella tabella clienti
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- Viste per join comuni (CORRETTE)
