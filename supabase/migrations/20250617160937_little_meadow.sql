/*
  ...
  4. Views
    - `fornitori_with_stats` with aggregated statistics from materiali_metallici
*/
...
-- Vista per fornitori con statistiche (ricrea sempre)
...
DO $$
BEGIN
  -- Aggiungi commenti solo se la tabella esiste
  ...
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'fornitori_with_stats') THEN
