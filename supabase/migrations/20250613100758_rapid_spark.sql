/*
  ...
  2. Security
    ...
    - User-specific policies for user table

  3. Automation
    - Auto-generate quote and job numbers
...
-- Quotes table - check if ditta column exists, if not add it
DO $$
BEGIN
  ...
  -- Add ditta column if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'preventivi' AND column_name = 'ditta'
  ) THEN
    ALTER TABLE preventivi ADD COLUMN ditta text NOT NULL DEFAULT 'alcafer';
  END IF;
END $$;

-- Jobs table - check if columns exist, if not add them
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE lavori ADD COLUMN ditta text NOT NULL DEFAULT 'alcafer';
  END IF;
END $$;
...
-- Add constraints after tables are created, checking column existence
DO $$
BEGIN
  -- Users constraints
  IF NOT EXISTS (
  ...
  -- Only add ditta constraint if column exists
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'preventivi' AND column_name = 'ditta'
  ) AND NOT EXISTS (
...
-- Create indexes for ditta columns only if they exist
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
