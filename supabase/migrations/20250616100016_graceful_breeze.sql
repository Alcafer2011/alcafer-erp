/*
  ...
  1. New Tables
    ...
    - `lavori` - Jobs with auto-generated numbers
...
DROP VIEW IF EXISTS preventivi_with_cliente;
DROP VIEW IF EXISTS lavori_with_preventivo;

-- Recreate views
CREATE VIEW preventivi_with_cliente AS
SELECT 
  p.*,
...
CREATE VIEW lavori_with_preventivo AS
SELECT 
  l.*,
  p.numero_preventivo,
  p.cliente_id,
