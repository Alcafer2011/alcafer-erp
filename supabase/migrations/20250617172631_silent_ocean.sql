/*
  ...
  1. New Tables
    - `fornitori`
      ...
      - `valutazione` (integer, default 3, range 1-5)
      - `note` (text, optional)
...
-- Policy per utenti autenticati (se non esiste)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Authenticated users can access fornitori' 
...
-- Indici per performance (se non esistono)
...
-- Trigger per aggiornamento automatico di updated_at (se non esiste)
DO $$
BEGIN
  IF NOT EXISTS (
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM fornitori LIMIT 1) THEN
