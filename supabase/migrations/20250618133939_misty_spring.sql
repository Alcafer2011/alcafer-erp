/*
  # Fix Policies and Create Posa in Opera Table

  1. Changes
    - Drop and recreate policies for lavori table
    - Create utility functions for permission checks
    - Create posa_in_opera table with proper relations
    - Add appropriate indexes and RLS policies
  
  2. Security
    - Enable RLS on posa_in_opera table
    - Add policy for authenticated users
    - Add audit trigger if available
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'posa_in_opera' 
    AND policyname = 'Authenticated users can access posa_in_opera'
  ) THEN
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'audit_trigger_function') THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE tgname = 'audit_posa_in_opera' 
      AND tgrelid = 'posa_in_opera'::regclass
    ) THEN
