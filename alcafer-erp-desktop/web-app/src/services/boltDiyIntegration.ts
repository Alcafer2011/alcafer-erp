...
export class BoltDiyIntegrationService {
  ...
  private async handleError(errorReport: ErrorReport): Promise<void> {
    ...
    toast.error(`Errore rilevato: ${errorReport.message}`, {
      ...
      // Prova percorso locale: inoltra a AI locale (senza bolt.diy)
    try {
      const suggestion = 'Analizzato errore: posso svuotare cache o ricaricare la pagina.';
      if (this.config.autoFix) {
        await this.applyAutoFix({ type: 'clear_cache' });
      }
    } catch (error) {
      console.error('❌ Errore gestione locale:', error);
    }
  }
  ...
  async requestFix(description: string): Promise<void> {
    try {
      ...
      if (response?.fix) {
        ...
        toast((t) => '🤖 bolt.diy sta analizzando il problema...');
      }
